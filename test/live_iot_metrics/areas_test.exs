defmodule LiveMetrics.Coap.AreasTest do
  ...
  describe "areas" do
    test "create an area" do
      with {:ok, area} <-
             LiveMetrics.Area.create_area(%{name: "test area", description: "This is test area"}) do
        assert area != nil
    ...
    test "create area and assign node" do
      with {:ok, device} <- LiveMetrics.Nodes.get_or_insert("00:1A:2B:3C:4D:5E"),
           {:ok, area} <-
             LiveMetrics.Area.create_area(%{name: "test area", description: "This is test area"}) do
        case LiveMetrics.Area.add_node(area, [device]) do
          {:ok, updated_area} ->
